...
const BusSchedule = () => {
    ...
    const SeatIcon = ({ isSelected, isBlocked, blockType }) => (
        <div className="relative flex flex-col items-center">
            <svg
                viewBox="0 0 100 100"
                className={`w-12 h-12 cursor-pointer transition-colors duration-200 ${isSelected ? 'text-green-600' : 'text-gray-000'} ${isBlocked
                        ? blockType === 'all'
                            ? 'text-red-600'
                            : 'text-yellow-500'
                        : 'text-green-600'
    ...
    const handleSeatClick = (seatId) => {
        setBlockedSeats(prev => ({
            ...prev,
            [seatId]: {
                isBlocked: !prev[seatId]?.isBlocked || false,
                blockType: blockType.all ? 'all' : 'online'
            }
        }));
    };
