...
const BulkSeatTransfer = () => {
        ...
        api.get('admin/bulk-seat-transfer/get-all')
            .then(res => {
                ...
                if (id) {
                    ...
                    let fl = res.data.filter(s => s.id === id)
                    ...
                    if (fl[0]) {
                        ...
                        setFilterData(prevState => {
                            return {
                                ...prevState, depot: s.depot, schedule: s, data: dayjs(s.date)
                            }
                        })
    ...
    const handleSearch = () => {
        ...
        api.post('admin/bulk-seat-transfer/search', {
            ...
            if(res.data.schedule.old)setOld(res.data.schedule.old)
    ...
    const handleTransferSubmit = () => {
        ...
        api.post('admin/bulk-seat-transfer/transfer', {
            ...transferDetails, id: selectedBooking.id, scheduleId: selectedSchedule.id,
        })
            .then(res => {
                stopLoading(L)
            ...
            if (!selectedSeat) {
                ...
                if (selectedSeat.status !== "available") {
                    ...
                    selectedSeats.push({
                        ...
                        balanceToPay: selectedSeat.balanceToPay ?? '', ...selectedSeat
                    });
    ...
    const filteredList = allschedules
        .filter(schedule => dayjs(schedule.date)
            .isSame(dayjs(filterData.date), 'day') && schedule.depot === filterData.depot && schedule.forTransfer);
                            ...
                            <Autocomplete
                                ...
                                onChange={(event, value, reason) => {
                                    ...
                                    setTransferDetails(prevState => {
                                        return {
                                            ...prevState, oldSeatCost: selectedBooking.price,
                    ...
                    {showSeatLayout && (<Box sx={{marginTop: "80px"}}>
                        ...
                        <Box sx={{
                            ...
                            <Box sx={{display: "flex", justifyContent: "center", width: "100%", maxWidth: "800px"}}>
                                ...
                                {renderSeatLayout(old??selectedSchedule)}
                            </Box>
                        </Box>
