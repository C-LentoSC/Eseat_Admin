...
const BulkSeatTransfer = () => {
    ...
    const [selectedSeat] = useState(null);
    ...
    const exportToCSV = () => {
        ...
        const csvData = bookings.map(booking => [
            ...
            booking.name,
            booking.bookedBy,
            formatDateForCSV(booking.bookedDate),
            booking.status
        ]);
    ...
    const handleSeatHold = () => {
        if (selectedBooking) {
            const updatedBookings = bookings.map(booking =>
                booking.refNo === selectedBooking.refNo
                    ? { ...booking, status: 'Holded' }
                    : booking
            );
    ...
    // Handle search
    const handleSearch = () => {
        // Simulate API call for schedule details
        setScheduleDetails({
            depot: "Main Depot",
            route: "Colombo-Kandy",
            date: "2024-01-10",
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs={12} md={4}>
                                <DatePicker
                                    ...
                                    onChange={(newValue) => setFilterData({ ...filterData, date: newValue })}
                            ...
                            <Grid item xs={12} md={4}>
                                <Autocomplete
                                    ...
                                    onChange={(_, value) => setFilterData({ ...filterData, depot: value })}
