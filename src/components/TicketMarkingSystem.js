...
const TicketMarkingSystem = () => {
  ...
  const filteredTickets = tickets.filter((ticket) => {
    const depotMatch =
      !depot || ticket.depot.toLowerCase().includes(depot.toLowerCase());
    const scheduleMatch =
      !scheduleNo ||
      ticket.scheduleNo.toLowerCase().includes(scheduleNo.toLowerCase());
    const vCodeMatch =
      !vCode || ticket.vCode.toLowerCase().includes(vCode.toLowerCase());
  ...
  const handleExport = () => {
    ...
    const rows = filteredTickets.map((ticket) => [
      ...
      formatCSVField(
        ticket.seatNoDetails
          .map(
            (s) =>
              `${s.seatNo} : ${s.confirmed ? "confirmed" : "not confirmed"}`
          )
          .join(", ")
      ),
    ...
    link.setAttribute(
      "download",
      `ticket_marking_${dayjs().format("YYYY-MM-DD_HH-mm")}.csv`
    );
            ...
            <TableBody>
              {filteredTickets.map((ticket) => (
                <TableRow key={ticket.id}>
                  ...
                  <TableCell>
                    {ticket.seatNoDetails.map((s) => s.seatNo).join(", ")}
                  </TableCell>
                  ...
                  <TableCell align="right">
                    {allSeatsConfirmed(ticket) ? (
                      ...
                      <IconButton
                        onClick={() => handleViewSeats(ticket)}
                        color="primary"
                      >
                        <HourglassEmpty />
                      </IconButton>
